...
def get_timezone(message):
    ...
    if message.text == "❌ Отменить создание":
        ...
        bot.send_message(user_id, "❌ Отменено.", reply_markup=get_main_keyboard())
        return
    
    tz_found = parse_timezone(message.text)
    if not tz_found:
        tz_found = 'Europe/Moscow'
        bot.send_message(user_id, "❌ Не понял. Использую МСК.")
    else:
        bot.send_message(user_id, f"✅ Часовой пояс: {tz_found}")
    
    user_data[user_id]['timezone'] = tz_found
    msg = bot.send_message(
        user_id,
        "📝 **Шаг 2 из 5: Введите название**",
        parse_mode='Markdown',
...
def get_reminder_text(message):
    ...
    if len(message.text.strip()) < 1:
        msg = bot.send_message(user_id, "❌ Название не может быть пустым.", reply_markup=get_cancel_keyboard())
        bot.register_next_step_handler(msg, get_reminder_text)
        return
...
def get_reminder_datetime(message):
    ...
    bot.send_message(
        user_id,
        f"✅ Время: {dt_local.strftime('%d.%m.%Y %H:%M')}"
    )
    
    msg = bot.send_message(
        user_id,
        "⏱️ **Шаг 4 из 5: Интервал спама**",
        parse_mode='Markdown',
...
def get_spam_interval(message):
    ...
    interval_map = {'1 минута': 60, '5 минут': 300, '10 минут': 600, '30 минут': 1800, '1 час': 3600}
    ...
    if interval_text in interval_map:
        ...
        try:
            ...
            bot.send_message(user_id, "❌ Не понял. Попробуй еще раз:", reply_markup=get_interval_keyboard())
...
def get_repeat_type(message):
    ...
    if 'месяц' in choice:
        repeat_type = 'monthly'; repeat_name = 'каждый месяц'
    elif 'недел' in choice:
        repeat_type = 'weekly'; repeat_name = 'каждую неделю'
    elif 'день' in choice:
        repeat_type = 'daily'; repeat_name = 'каждый день'
    elif 'не повтор' in choice or 'один раз' in choice or 'нет' in choice:
        repeat_type = 'none'; repeat_name = 'не повторять'
    else:
        bot.send_message(user_id, "❌ Не понял. Выбери вариант:", reply_markup=get_repeat_type_keyboard())
    ...
    repeat_names = {'daily': '🔄 Каждый день', 'weekly': '📅 Каждую неделю', 'monthly': '📆 Каждый месяц', 'none': '❌ Не повторять'}
    
    bot.send_message(
        user_id,
        f"✅ **Напоминание создано!**\n\n"
        f"📝 {user_data[user_id]['text']}\n"
        f"📅 {dt_local.strftime('%d.%m.%Y %H:%M')}\n"
        f"⏱️ {minutes} мин\n"
        f"🔄 {repeat_names.get(repeat_type, '❌ Не повторять')}\n"
        f"🌍 {user_data[user_id]['timezone']}",
        parse_mode='Markdown',
        reply_markup=get_main_keyboard()
    )
...
def list_reminders(message):
    ...
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute("SELECT reminder_time, text, spam_interval, repeat_type, timezone FROM reminders WHERE user_id = ? AND is_done = 0", (user_id,))
    rows = c.fetchall()
    conn.close()
    
    if not rows:
        bot.send_message(user_id, "📭 Нет активных напоминаний.", reply_markup=get_main_keyboard())
        return
    
    text = "📋 **Твои напоминания:**\n\n"
    for i, (rem_time, rem_text, interval, repeat_type, timezone) in enumerate(rows, 1):
        try:
            tz = pytz.timezone(timezone)
        ...
        repeat_names = {'daily': '🔄 Ежедневно', 'weekly': '📅 Еженедельно', 'monthly': '📆 Ежемесячно', 'none': '❌ Одноразовое'}
        text += f"{i}. 📝 {rem_text}\n   📅 {rem_time_local}\n   ⏱️ {interval//60} мин\n   {repeat_names.get(repeat_type, '❌ Одноразовое')}\n\n"
...
def delete_reminder(message):
    ...
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute("SELECT id, reminder_time, text FROM reminders WHERE user_id = ? AND is_done = 0", (user_id,))
    rows = c.fetchall()
    conn.close()
    ...
    keyboard = InlineKeyboardMarkup(row_width=1)
    for rem_id, rem_time, rem_text in rows:
        keyboard.add(InlineKeyboardButton(f"🗑️ {rem_text[:20]}", callback_data=f"delete_{rem_id}"))
    keyboard.add(InlineKeyboardButton("❌ Отмена", callback_data="delete_cancel"))
    
    bot.send_message(user_id, "Выбери напоминание:", reply_markup=keyboard)
...
def handle_done(call):
    ...
    if result:
        ...
        if repeat_type != 'none':
            ...
            bot.send_message(call.message.chat.id, "Выбери действие:", reply_markup=get_repeat_keyboard(reminder_id))
        else:
            mark_reminder_done(reminder_id)
            bot.answer_callback_query(call.id, "✅ Отлично!")
            bot.edit_message_text(f"✅ **ВЫПОЛНЕНО:** {text}", chat_id=call.message.chat.id, message_id=call.message.message_id, parse_mode='Markdown')
            bot.send_message(call.message.chat.id, "🎉 Спам остановлен!", reply_markup=get_main_keyboard())
...
def handle_repeat(call):
    ...
    if choice == "yes":
        ...
        if result:
            ...
            c.execute("""INSERT INTO reminders (user_id, reminder_time, text, spam_interval, last_spam_time, repeat_type, timezone) 
                         VALUES (?, ?, ?, ?, ?, ?, ?)""",
                      (user_id, next_time, text, spam_interval, datetime.now().strftime("%Y-%m-%d %H:%M:%S"), repeat_type, timezone))
            c.execute("UPDATE reminders SET is_done = 1 WHERE id = ?", (reminder_id,))
            conn.commit()
            
            bot.send_message(
                user_id,
                f"🔄 **Продлено!**\n📝 {text}\n📅 {next_dt.strftime('%d.%m.%Y %H:%M')}",
                parse_mode='Markdown',
                reply_markup=get_main_keyboard()
            )
        conn.close()
        ...
        mark_reminder_done(reminder_id)
        bot.answer_callback_query(call.id, "⏹️ Остановлено!")
        bot.delete_message(call.message.chat.id, call.message.message_id)
        bot.send_message(user_id, "✅ Повтор остановлен.", reply_markup=get_main_keyboard())
            ...
            for rem_id, user_id, text, spam_interval, last_spam_time, repeat_type, timezone, reminder_time in active_reminders:
                ...
                if seconds_since_last_spam >= spam_interval:
                    try:
                        ...
                        repeat_names = {'daily': '🔄 Ежедневное', 'weekly': '📅 Еженедельное', 'monthly': '📆 Ежемесячное', 'none': '❌ Одноразовое'}
